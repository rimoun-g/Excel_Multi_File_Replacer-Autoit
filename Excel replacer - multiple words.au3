...
;~ appload section
 MsgBox (48, "Warning", "please save and close your Excel files to avoid any issues")



#Region ### START Koda GUI section ### Form=
$frmMainForm = GUICreate("Replace text in Excel files", 1100, 600)
...
$chkbxMatchEntireCell = GUICtrlCreateCheckbox("Match entire cell contents", 720, 150, 200, 25)
...
While 1
	...
	Switch $nMsg
		...
		Case $btnSelect
			...
			$G = FileSelectFolder ("Choose Excel folder", @ScriptDir)
			 GUICtrlSetData($FolderPath,$G)
			 GetFilesList($G,"*.xlsx;*.xls", _IsChecked($idCheckbox)) ;add extensions as needed separated by semicolon
		Case $Del
	...
	if $FPath <> "" Then
		...
		if UBound ($FileList) > 1 Then
			...
			For $i = 1 To $FileList[0]
				if StringInStr($FileList[$i],"~$") = 0 Then GUICtrlCreateListViewItem($FileList[$i],$lstFiles)
			Next
...
Func ReplaceWords()
Local $MatchType = Default
if _IsChecked($chkbxMatchEntireCell) Then $MatchType = $xlWhole
Local $countItems = _GUICtrlListView_GetItemCount($lstFiles)
Local $File
if  $countItems > 0 Then
		GUICtrlSetState($btnReplaceWords, 128)
		$oExcel = _Excel_Open(False, Default, Default, Default, True) ;opening word application
	...
	For $i = 0 To $countItems - 1
		...
		$oWorkBook = _Excel_BookOpen($oExcel, $File, Default,Default, False)
		ConsoleWrite($File)
		Local $SheetCount = $oWorkBook.Worksheets.Count

		For $sheet = 1 To $SheetCount
			If @error Then  MsgBox(16+48, "Error","Error opening the document." & @CRLF & "@error = " & @error & ", @extended = " & @extended)
				...
				For $j = 0 To _GUICtrlListView_GetItemCount ( $lstWords ) -1 ;loobs through the list of replacements to perform all of them in each document
				   $oRangeFound = _Excel_RangeReplace ($oWorkBook, $sheet, Default, _GUICtrlListView_GetItemText($lstWords,$j), _GUICtrlListView_GetItemText($lstWords,$j,1), $MatchType)

				Next
		Next
		_Excel_BookClose ($oWorkBook, True) ;closed the word document
		GUICtrlSetData($lblGettingItems,"Replacing..." & Int((($i+1)/($countItems))*100) & "%")
	Next

	_Excel_Close ($oExcel,Default,True) ; closes the word application
...
Func AddTextFromExcel($ExcelFile)
	 $oExcel = _Excel_Open(False, Default, Default, Default, True) ; creates xl application
...
With $oWorkbook.ActiveSheet ; process active sheet
	...
	_Excel_Close($oExcel, False, True) ; close the xl file
